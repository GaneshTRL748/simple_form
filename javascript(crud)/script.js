...
{  
    ...
    while(temp!=null)
    {
       ...
       for( i=0;i<len;i++)
       {
          (i<len-1)?row.insertCell(i).innerHTML=data[i]:row.insertCell(i).innerHTML=`<button onclick="editdata(this)">edit</button> <button onclick="deletedata(this)">delete</button>`;
       }
       temp=temp.next;
     }  
...
deletedata=(id)=>
{
    ...
    if(check==true){
    emp=id.parentElement.parentElement.cells[0].innerHTML;
    let temp=head;
    let temp1;
    while(temp!=null)
    {
    ...
    (temp==head)? head=temp.next:temp1.next=temp1.next.next;
    display();
}
}
editdata=(id)=>
